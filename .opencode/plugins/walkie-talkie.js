...
function generateQRCode(url) {
  return new Promise((resolve) => {
    import_qrcode_terminal.default.generate(url, { small: true }, resolve);
  });
}
...
var WALKIE_XML_RE = /<walkie-connection ip="([^"]+)" port="(\d+)" token="([^"]*)"\s*\/>/;
var index_default = async ({ client }) => {
  ...
  return {
    tool: {
      walkie_qr: {
        ...
        async execute() {
          ...
          return `<walkie-connection ip="${ip}" port="${port2}" token="${token}" />`;
        }
      }
    },
    event: async ({ event }) => {
    ...
    "experimental.text.complete": async (_input, output) => {
      ...
      if (!match) return;
      const [fullMatch, ip, port2, token] = match;
      const wsUrl = token ? `wss://${ip}:${port2}/${token}` : `wss://${ip}:${port2}`;
      const httpUrl = `https://${ip}:${port2}`;
